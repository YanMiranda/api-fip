...
CREATE TABLE VEICULOS (
                          ...
                          MA_ID                 NUMBER(19) NOT NULL,
                          MO_ID                 NUMBER(19) NOT NULL,
                          VE_ANO_MODELO         NUMBER(10) NOT NULL,
                          VE_COMBUSTIVEL        VARCHAR2(50 CHAR),
                          VE_CODIGO_FIPE        VARCHAR2(20 CHAR),
                          VE_VALOR              VARCHAR2(50 CHAR),
                          VE_OBSERVACOES        VARCHAR2(500 CHAR),
...
CREATE INDEX IX_VEICULOS_MA_ID ON VEICULOS (MA_ID);
CREATE INDEX IX_VEICULOS_MO_ID ON VEICULOS (MO_ID);
CREATE UNIQUE INDEX UK_VEICULOS_COD_FIPE ON VEICULOS (VE_CODIGO_FIPE);

COMMENT ON TABLE VEICULOS IS 'Cadastro de veículos da tabela FIPE';
COMMENT ON COLUMN VEICULOS.VE_ID IS 'Identificador do veículo';
COMMENT ON COLUMN VEICULOS.MA_ID IS 'FK para MARCAS.MA_ID';
COMMENT ON COLUMN VEICULOS.MO_ID IS 'FK para MODELOS.MO_ID';
COMMENT ON COLUMN VEICULOS.VE_ANO_MODELO IS 'Ano do modelo do veículo';
COMMENT ON COLUMN VEICULOS.VE_COMBUSTIVEL IS 'Tipo de combustível';
COMMENT ON COLUMN VEICULOS.VE_CODIGO_FIPE IS 'Código FIPE do veículo (único)';
COMMENT ON COLUMN VEICULOS.VE_VALOR IS 'Valor conforme tabela de referência';
COMMENT ON COLUMN VEICULOS.VE_OBSERVACOES IS 'Observações sobre o veículo';
